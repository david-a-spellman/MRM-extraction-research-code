...
def findArgsAroundRV (rv, rvs, index, graph, title, indicies = None):
	json_graph = regulationRelationToJSON (rv, index, graph)
	print (json_graph)
	tokens = word_tokenize (title)
	#print (tokens)
	tok_idx = list ()
...
def back_track (json_graph, last_keys, amount):
	fix = json_graph
	l = len (last_keys)
	for i in range (0, (l - 1)):
		if not (last_keys [i] in fix):
			print (last_keys)
			print (fix)
			print (amount)
		fix = fix [last_keys [i]]
	fix [last_keys [-2]] = last_keys [-1]
	temp = json_graph
	for i in range (0, (l - amount)):
		temp = temp [last_keys [i]]
	for i in range (-1, (l - amount), -1):
		last_keys.pop (i)
	return temp, (l - 1 - amount), last_keys

def regulationRelationToJSON (rv, index, graph):
	depth = 0
	closures = 0
	# Track number of spaces of indentation on previous line of graph
	first_spaces = 0
	first_line = graph [index]
	for c in first_line:
		if c == " ":
			first_spaces += 1
		else:
	...
	if first_spaces > 0:
		first_indents = 1
	print (str ("first spaces: " + str (first_spaces)))
	print (str ("first indents: " + str (first_indents)))
	prev_indents = first_indents
	index += 1
	# Track references to words that are a single character potentially followed by a single number
	refferences = dict ()
	json_graph = dict ()
	json_graph [rv] = dict ()
	temp = json_graph [rv]
	...
	last_key = 0
	past_indents = dict ()
	past_indents [first_spaces] = first_indents
	spaces = first_spaces
	indents = first_indents
	for i in range (index, len (graph)):
		line = graph [i]
		word = ""
		# Set this flag once the characters that need extracted are reached
		# unset this flag once the characters that need extracted are passed
		scanning = False
		prev_spaces = spaces
		# Spaces may prove the most reliable way to capture the tree structure from the AMR output
		spaces = 0
		prev_indents = indents
		# calculate from spaces and prev_spaces
		indents = 0
		node = False
		# capturing node type that can be arg, mod, op, mechanism, protein, exetra
		nt = ""
		# is at first set to true in order to indicate the start of a line so white space can be counted
		start = True
		# For capturing word refferences that consist of a letter and possibly digit
		reff = False
		refference = ""
		for j in range (0, len (line)):
			...
			if c == "(":
				...
				if start:
					...
					node = False
					continue
			elif c.isnumeric () and node:
				node = False
				continue
			elif c.isnumeric () and reff:
				reff = False
				refference += c
				continue
			elif c.isalpha () and node:
				nt += c
				continue
			elif c.isalpha () and reff:
				refference += c
				continue
			elif c.isalpha () and scanning:
				word += c
				continue
			elif c == "/":
				scanning = True
				...
				scanning = False
				continue
		# Now implement logic for processing the content of each line after it has been scanned character by character
		if (spaces - prev_spaces) > 0:
			indents = (prev_indents + 1)
			if not (spaces in past_indents):
				past_indents [spaces] = indents
		elif (spaces - prev_spaces) == 0:
			indents = prev_indents
		else:
			indents = past_indents [spaces]
		#print (str ("spaces: " + str (spaces)))
		print (str ("indents: " + str (indents)))
		if indents == first_indents:
			new_index = i
			break
		elif indents > prev_indents and not (nt in spec_nodes) and not (word in category_words):
			temp [str (refference + ":" + word)] = dict ()
			temp = temp [str (refference + ":" + word)]
			last_keys.append (str (refference + ":" + word))
			last_key += 1
			continue
		elif indents == prev_indents:
			temp, last_key, last_keys = back_track (json_graph, last_keys, 1)
			temp [str (refference + ":" + word)] = dict ()
			temp = temp [str (refference + ":" + word)]
			last_keys.append (str (refference + ":" + word))
			last_key += 1
			...
			amount = int (prev_indents - indents)
			temp, last_key, last_keys = back_track (json_graph, last_keys, amount)
			temp [str (refference + ":" + word)] = dict ()
			temp = temp [str (refference + ":" + word)]
			last_keys.append (str (refference + ":" + word))
			last_key += 1
